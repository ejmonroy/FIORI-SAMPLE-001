			...
			actions : {
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("OrderNumber") === oObject.OrderNumber;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].OrderNumber;
									} else {
										sCurrentId = aItemsNotInTheList[0].OrderNumber;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("AllocationHeaderSet", {
										OrderNumber : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("OrderNumber"),
						title: oBindingContext.getProperty("OrderDesc")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						viewName : sViewName,
						id : "page",
						matchers : new PropertyStrictEquals({name : "title", value : "<AllocationHeaderSet> (0)"}),
						success : function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("OrderDesc");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all AllocationHeaderSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "page",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "title", value : "<AllocationHeaderSet> (" + iExpectedLength + ")"}),
								success : function () {
